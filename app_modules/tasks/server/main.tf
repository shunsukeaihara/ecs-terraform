...
data "template_file" "container_definitions" {
  template = file("${path.module}/../../templates/server_task.tpl.json")
  ...
  vars = {
    ...
    nginx_enviroment  = jsonencode(var.nginx_enviroment)
    nginx_log_driver  = var.nginx_log_driver
    nginx_log_options = jsonencode(var.nginx_log_options)
    nginx_host_port   = var.nginx_host_port
    nginx_command     = jsonencode(var.nginx_command)
